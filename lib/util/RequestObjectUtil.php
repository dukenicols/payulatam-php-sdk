...
class RequestObjectUtil {
		...
		if(is_object($object)){
			foreach($object as $k => $v){
				if($v === null){
					...
					$tempObject = RequestObjectUtil::removeNullValues($object->$k);
					if(!isset($tempObject)){
						unset($object->$k);
					}else{
						$object->$k = $tempObject;
		...
		if(is_object($object)){
			foreach($object as $k => $v){
				if($v != null && is_string($v)  && mb_detect_encoding($v,'UTF-8') != 'UTF-8'){
					...
					$object->$k = RequestObjectUtil::encodeStringUtf8($object->$k);
				}
			}
		}else if(is_array($object)){
			foreach($object as $k => $v){
	...
	public static function formatDates($data){
		...
		foreach($data as $k => $v){
			if(RequestObjectUtil::isKeyDateField($k)){
				if(is_array($data)){
					$miliseconds = $data[$k];
					$data[$k] = RequestObjectUtil::getDate($miliseconds);
				}else if(is_object($data)){
					$miliseconds = $data->$k;
					$data->$k = RequestObjectUtil::getDate($miliseconds);
				}
			}else if(is_object($data) && (is_object($data->$k) || is_array($data->$k))){
				$data->$k = RequestObjectUtil::formatDates($data->$k);
