...
use PayU\Util\CommonRequestUtil as CommonRequestUtil;
use PayU\Api\KeyMapName as PayUKeyMapName;
use PayU\Api\HttpRequestInfo as PayUHttpRequestInfo;
use PayU\Api\Environment as Environment;
use PayU\Util\SubscriptionsUrlResolver as PayUSubscriptionsUrlResolver;
...
class SubscriptionsRequestUtil extends CommonRequestUtil{
	...
	public static function buildSubscription($parameters, $existParamBankAccount=FALSE, $existParamCreditCard=FALSE, $edit = FALSE){
		...
		if ($edit == TRUE){
			...
			$customer = SubscriptionsRequestUtil::buildCustomer($parameters);
			...
			$subscription->plan = SubscriptionsRequestUtil::buildSubscriptionPlan($parameters);
	...
	protected static function buildCreditCardForSubscription($parameters){
		...
		if(!isset($tokenId)){
			$creditCard = SubscriptionsRequestUtil::buildCreditCard($parameters);
	...
	public static function buildSubscriptionPlan($parameters){
		...
		$subscriptionPlan->additionalValues = SubscriptionsRequestUtil::buildSubscriptionPlanAdditionalValues($planCurrency, $planValue, $planTaxValue, $planTaxReturnBase);
	...
	public static function buildCreditCard($parameters){
		...
		$creditCard->address = SubscriptionsRequestUtil::buildAddress($parameters);


		$expirationDate = CommonRequestUtil::getParameter($parameters, PayUParameters::CREDIT_CARD_EXPIRATION_DATE);

		if (isset($expirationDate)){
			SubscriptionsRequestUtil::isValidDate($expirationDate,
	...
	private static function buildSubscriptionPlanAdditionalValues($planCurrency, $planValue, $planTaxValue, $planTaxReturnBase){
		...
		$additionalValues = SubscriptionsRequestUtil::addAdditionalValue($additionalValues, $planCurrency, PayUKeyMapName::PLAN_VALUE, $planValue);

		$additionalValues = SubscriptionsRequestUtil::addAdditionalValue($additionalValues, $planCurrency, PayUKeyMapName::PLAN_TAX, $planTaxValue);

		$additionalValues = SubscriptionsRequestUtil::addAdditionalValue($additionalValues, $planCurrency, PayUKeyMapName::PLAN_TAX_RETURN_BASE, $planTaxReturnBase);
	...
	private static function buildItemAdditionalValues($currency, $value, $taxValue, $taxReturnBase){
		...
		$additionalValues = SubscriptionsRequestUtil::addAdditionalValue($additionalValues, $currency, PayUKeyMapName::ITEM_VALUE, $value);

		$additionalValues = SubscriptionsRequestUtil::addAdditionalValue($additionalValues, $currency, PayUKeyMapName::ITEM_TAX, $taxValue);

		$additionalValues = SubscriptionsRequestUtil::addAdditionalValue($additionalValues, $currency, PayUKeyMapName::ITEM_TAX_RETURN_BASE, $taxReturnBase);
	...
	public static function buildRecurringBillItem($parameters){
		...
		$recurringBillItem->additionalValues =
								SubscriptionsRequestUtil::buildItemAdditionalValues(
																	$currency,
																	$itemValue,
																	$itemTaxValue,
	...
	public static function validateCustomerToSubscription($parameters, $edit = FALSE){
		...
		if(isset($customerId) || $edit == TRUE){
			...
			SubscriptionsRequestUtil::validateCustomer($parameters, FALSE);
		}
